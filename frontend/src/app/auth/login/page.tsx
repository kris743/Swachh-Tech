...
export default function LoginPage() {
  ...
  return (
    <div className="flex min-h-screen bg-background text-foreground">
      ...
      <div className="hidden lg:flex w-1/2 relative bg-emerald-900 overflow-hidden items-center justify-center p-12">
        <div className="absolute inset-0">
          <div className="absolute inset-0 bg-gradient-to-br from-emerald-600/40 via-emerald-900/80 to-background z-10" />
        ...
        <motion.div 
          ...
          className="relative z-20 max-w-lg text-white"
        >
          <div className="p-4 bg-white/10 backdrop-blur-xl rounded-2xl inline-flex items-center justify-center mb-8 border border-white/20 shadow-2xl">
          ...
          <p className="text-lg text-emerald-100/80 mb-10 leading-relaxed font-medium">
            Join the revolution in smart waste management. Track, gamify, and optimize the way your community handles recycling.
          </p>
            ...
            <div className="flex -space-x-4">
              {[1, 2, 3, 4].map((i) => (
                <div key={i} className="w-10 h-10 rounded-full border-2 border-emerald-900 bg-emerald-800 flex items-center justify-center overflow-hidden">
                  <div className="w-full h-full bg-gradient-to-br from-emerald-400 to-blue-500 opacity-70"></div>
                </div>
              ))}
            </div>
        ...
        >
          ...
          <div className="grid grid-cols-2 gap-4 mb-8">
            <button 
              ...
              disabled={isLoading}
              className="flex items-center justify-center gap-2 py-2.5 border border-border rounded-xl font-semibold hover:bg-muted/50 transition-colors disabled:opacity-50"
            >
            ...
            <button className="flex items-center justify-center gap-2 py-2.5 border border-border rounded-xl font-semibold hover:bg-muted/50 transition-colors">
          ...
          <form className="space-y-6" onSubmit={handleLogin}>
            <div className="space-y-5">
              <div className="group relative">
                <label className="block text-sm font-bold mb-2 text-foreground/90">Email Address</label>
                <div className="relative">
                  ...
                  <input
                    ...
                    className="block w-full rounded-xl border border-border bg-muted/30 pl-11 pr-4 py-3.5 text-foreground placeholder:text-muted-foreground/60 focus:border-emerald-500 focus:bg-background focus:outline-none focus:ring-4 focus:ring-emerald-500/10 transition-all font-medium"
              ...
              <div className="group relative">
                <div className="flex justify-between items-center mb-2">
                  <label className="block text-sm font-bold text-foreground/90">Password</label>
            ...
            <div className="flex items-center mt-4 mb-8">
              <input
                id="remember-me"
                type="checkbox"
                className="h-4.5 w-4.5 rounded border-border bg-muted/50 text-emerald-500 focus:ring-emerald-500 focus:ring-offset-background transition-colors cursor-pointer"
              />
              <label htmlFor="remember-me" className="ml-3 block text-sm font-medium text-foreground/80 cursor-pointer select-none">
            ...
            <button 
              type="submit" 
              disabled={isLoading}
              className="group relative flex w-full justify-center items-center rounded-xl bg-emerald-500 px-4 py-4 text-base font-bold text-white hover:bg-emerald-600 focus:outline-none focus:ring-4 focus:ring-emerald-500/20 disabled:opacity-70 transition-all shadow-xl shadow-emerald-500/20 hover:-translate-y-0.5 hover:shadow-emerald-500/40"
            >
              {isLoading ? (
          ...
          <div className="text-center mt-10">
            ...
            <Link href="/auth/register" className="font-bold text-emerald-600 hover:text-emerald-500 transition-colors ml-1">
              Create an account
            </Link>
          </div>
        </motion.div>
